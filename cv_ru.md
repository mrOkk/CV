# Резюме - Олег Копосов
## Lead/Senior Game Developer

## Контакты
- Локация: Россия
- Формат: гибрид / удаленно
- LinkedIn: https://www.linkedin.com/in/ok-kop/
- Telegram: https://t.me/mr_okk

## Профиль
Lead/Senior Unity-разработчик с опытом в multiplayer, gameplay, UI и meta-системах. Руководил UI-группой, выстраивал процессы разработки, проводил code/performance review и менторил разработчиков. Сильный фокус на боевых системах, производительности и кросс-функциональном взаимодействии.

## Ключевая экспертиза
- Leadership: развитие команды, ревью, техническое наставничество, синхронизация со смежными отделами
- Unity/C#: gameplay architecture, UI architecture, оптимизация производительности
- Multiplayer: клиентская логика, интеграция с backend, socket-взаимодействие
- Meta и LiveOps: прогрессия, награды, контентные системы, внутренние тулзы
- UI: UGUI/NGUI, анимации интерфейса, DOTween

## Опыт работы
### OneUp Games - Unity Developer
- Период: Dec 2022 - Present
- Проект: Urban Heat (мобильный онлайн-шутер)
- Контекст: разработка динамичного PvP-шутера с акцентом на скилл игрока
- Зона ответственности:
  - Разработка core-геймплея: расходники, перки, способности, ультимейты, ближний бой
  - Визуальная gameplay-логика: VFX-события (пули, взрывы, трассеры), сборка/отрисовка игрока и оружия
  - UI/HUD: верстка, логика, анимации на DOTween, оптимизация производительности
  - Системы вовлечения: оценка эффективности в бою и награды за ключевые достижения
  - Техническая поддержка продукта: миграции фич, разработка и поддержка внутренних инструментов
  - Поддержка meta-фич как со стороны core, так и как полноценный feature developer

### Siridar - UI Programming Team Lead
- Период: Mar 2022 - Nov 2022
- Проект: мобильная online survival RPG
- Зона ответственности:
  - Руководство группой UI-разработки
  - Планирование и delivery фич
  - Взаимодействие с art, backend, development, продюсерами и менеджментом
  - Code review и performance review
  - Менторство и онбординг сотрудников
  - Выстраивание эффективного процесса разработки в период миграции между системами трекинга задач

### Red Rift, Inc. - Senior Unity Developer
- Период: Nov 2021 - Mar 2022
- Проект: Puzzle Royale (mobile match-3 battler)
- Зона ответственности:
  - Реализация core-логики match-3
  - Разработка socket-слоя для взаимодействия с backend
  - Интеграция Spine-анимации героев
  - Разработка live-ops системы для масштабируемого добавления героев
  - Интеграция VFX для боевых действий
  - Кросс-функциональная коммуникация с продактом, геймдизайном, артом и backend
  - Наставничество junior-разработчиков

### Battlestate Games - Unity Developer
- Период: Jun 2018 - Nov 2021
- Проект: Escape from Tarkov
- Зона ответственности:
  - Разработка meta-направления: барахолка (p2p-магазин), убежище, квесты, инвентарь, кастомизация
  - Подключение к задачам core-геймплея по мере необходимости

### Versus Games - Unity Developer
- Период: Dec 2016 - May 2018
- Проект: сессионный экшен
- Зона ответственности:
  - Реализация общей игровой логики
  - Разработка UI на NGUI
  - Интеграция анимаций и IK (FinalIK)
  - Поддержка и доработка проектных систем
  - Участие в задачах по БД и backend
  - CI/CD: GitLab + TeamCity

### Дополнительный опыт
- DIRECTUM (Xamarin Developer, .NET/WPF Developer, Developer)
- Nosorog Studio (Mobile Developer)
- Center of High Technologies (Windows Phone Developer)

## Образование
- Izhevsk State Technical University (ISTU)
- Specialist, Comprehensive Information Security of Automated Systems

## Сертификации
- Microsoft: XAML Deep Dive for Windows & Windows Phone Apps Jump Start
- Microsoft: MCTS 70-506 (Silverlight 4, Development)

## Языки
- Русский
- Английский

## Дополнительно
Люблю иногда поучаствовать в гейм джемах
- LD profile: https://ldjam.com/users/okk/